...
How does the provided code meet the specification?
 ...
 - "The vending machine must track the following state: Available items – each item has a count, a price, and a selector (A, B or C)"
Machine HAS-A product for tracking. Tracking is enabled for Product in the Products class. This is a singleton class with static list. Refer to JUnit ProductsTest for demo.

 - "The vending machine must track the following state: Currently inserted money"
Machine HAS-A coin for tracking. currently inserted money can be retrieved at any point of a state from the getter. The test cases are spread across.
 ...
 - "The vending machine must track the following state: Whether the machine is on or off"
